...
DirichletBoundaryValue(find(x(:,1)==-1)) = sin(pi*t).*exp(-10*(1 + x(find(x(:,1)== -1),2).^2));  %% Along x = -1
DirichletBoundaryValue(find(x(:,1)== 1)) = sin(pi*t).*exp(-10*(x(find(x(:,1)==1),2).^2 + 1));  %% Along x = 1
DirichletBoundaryValue(find(x(:,2)== -1)) = sin(pi*t).*exp(-10*(1 + x(find(x(:,2)==-1),1).^2));  %% Along y = -1
DirichletBoundaryValue(find(x(:,2) == 1)) = sin(pi*t).*exp(-10*(1 + x(find(x(:,2)== 1),1).^2));  %% Along y = 1
