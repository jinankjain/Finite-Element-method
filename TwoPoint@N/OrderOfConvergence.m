...
for k = 1:NoIteration
    ...
    [U,A,B,ndof] = TwoPoint(c4n,n4e,unique(n4sDb),N(k,1),dt(k,1));
		spaceestimate1(k) = dt(k,1)*timelevelspaceestimator1(c4n,n4e,n4sDb,T,N(k,1),dt(k,1),U);
		timereconesti1(k) = dt(k,1)*timeleveltimerecestimator1(c4n,n4e,n4sDb,T,N(k,1),dt(k,1),U);
		timereconesti2(k) = max(timeleveltimerecestimator2(c4n,n4e,n4sDb,T,N(k,1),dt(k,1),U));
		maxl2reconsesti(k) = max(timelevell2spaceestimate(c4n,n4e,n4sDb,T,N(k,1),dt(k,1),U));
		l1spaceesti(k) = dt(k,1)*timelevelspaceestimator(c4n,n4e,n4sDb,T,N(k,1),dt(k,1),U);
